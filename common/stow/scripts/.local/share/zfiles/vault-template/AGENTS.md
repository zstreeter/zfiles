...
```
quarto render notes/x.md --to typst    # paper: PDF via Typst, no TeX needed
quarto render notes/x.md --to pdf      # the same through lualatex, when a venue needs it
...
`quarto render notes/x.md --to typst 2>&1 | grep obsidian.lua`.

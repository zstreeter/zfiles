...
INSTALL_DIR="-Dprefix=$HOME/.local"
...
pushd neovim
make CMAKE_BUILD_TYPE=Release CMAKE_EXTRA_FLAGS="-DCMAKE_INSTALL_PREFIX=$HOME/.local/bin"
make install
popd
...
pushd nwg-launchers
meson "$INSTALL_DIR" builddir -Dbuildtype=release
ninja -C builddir
popd
...
# lf
sudo apt install -y golang-go                                               # requires go
